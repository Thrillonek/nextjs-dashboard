				...
				<div className='bg-white px-6'>
					{latestInvoices.map((invoice, i) => {
						return (
							<div
								key={invoice.id}
								className={clsx('flex flex-row justify-between items-center py-4', {
									'border-t': i !== 0,
								})}
							>
								<div className='flex items-center'>
									...
									<div className='min-w-0'>
										<p className='font-semibold text-sm md:text-base truncate'>{invoice.name}</p>
										<p className='hidden sm:block text-gray-500 text-sm'>{invoice.email}</p>
									</div>
								</div>
